...
Sometimes when programming we are given problems where we have to define multiple patterns of behavior based on met conditions. We going to 
continue using the *movie theater* problem to explore this. 

**Problem:**

The local movie theatre has noticed that some small children do not enjoy watching *The Princess Bride*. The movie theatre wants to offer tickets
...
In Go, and most programming language, there is a construct called [`Else If`](https://gobyexample.com/if-else) that allows us to write this is a more readable way. `Else If` allows us to add additional condition checks to an `If Els` statement. For example: 
...
func main() {
	movies := getMovies(6)
	fmt.Printf("Here is yor movie list based on age: %v+", movies)
}
```
